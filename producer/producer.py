...
def main() -> None:
    ...
    for driver in cfg.driver_numbers:
        ...
        year = "unknown"
        session = f"sk{cfg.session_key}"

        topic_prefix = f"{cfg.base_topic}/{year}/{session}/{driver}"
        print(f"[INFO] replay driver={driver} car_rows={len(car_rows)} loc_rows={len(loc_rows)} speedup={cfg.replay_speedup}")

        publish_stream(client, topic_prefix, "telemetry", car_rows, cfg.replay_speedup)
        publish_stream(client, topic_prefix, "location", loc_rows, cfg.replay_speedup)
